...
CREATE OR REPLACE FUNCTION restore_from(ip character varying, jbossporttienda integer, jbossportcentral integer, connectbusiness integer,businesscentral integer,userdb character varying, passdb character varying, delegation character varying, warehouse character varying, installationNumber integer, franquicia boolean, solo_mi_destino boolean)
  RETURNS boolean AS
$BODY$
DECLARE
...
SET CONSTRAINTS ALL DEFERRED; 
	...
	businesslocal=1;
	EXECUTE 'SELECT * FROM dblink_connect(''host='||ip||'
                                  port=5432
                                  user='||userdb||'
                                  password='||passdb||'
	...
	LOOP
		...
		IF position('#' in tables.table_name)<>0 THEN
			IF tables.table_name<>'módulo_negocio#propiedades' AND tables.table_name<>'módulo_negocio#clases' THEN
			...
			END IF;	
		ELSIF tables.table_name<>'deleted_objects' AND tables.table_name<>'log_error' AND position('replica_' in tables.table_name)<>1 THEN
	...
	DELETE FROM "línea_artículos_materia" as e where "albarán_clienteId" is not null and "albarán_clienteId" not in(select "tableId" from "albarán_cliente");
	...
	INSERT INTO replica_send_get(autonum_replica_msg,source,msguid,fecha) SELECT maxautonum,central,msguid,current_date FROM dblink('SELECT autonum,msguid,del.rdn as central FROM replica_msg,"aplicación" as ap inner join "delegación" as del on(ap."delegación"=del."tableId") WHERE autonum=(select max(autonum) FROM replica_msg)')  AS remotetable (maxautonum integer,msguid character varying,central character varying);
	...
	EXECUTE 'INSERT INTO replica_destination_post(destination,ip,business,port,portjboss) VALUES('''||delegationCentralRdn||''','''||ip||''','||businesscentral||',5432,'||jbossportcentral||')';
	insert into replica_source_get(source,ip,business,port,portjboss,"user",password) values(''||delegationCentralRdn||'',ip,businesscentral,5432,jbossportcentral,userdb,passdb);
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION restore_from(ip character varying, jbossporttienda integer, jbossportcentral integer, connectbusiness integer, businesscentral integer, userdb character varying, passdb character varying, delegation character varying, warehouse character varying, installationNumber integer, franquicia boolean,boolean) OWNER TO dynagent;
