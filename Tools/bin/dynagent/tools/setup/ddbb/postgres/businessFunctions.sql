...
CREATE OR REPLACE FUNCTION comisiones(agente integer[], param_fecha_inicio bigint, param_fecha_fin bigint)
...
FROM
	...
	( 	"v_exclusividad_comercial#agente_comercial" as excag 																					inner join
		exclusividad_comercial as exc		on(exc."tableId"=excag."exclusividad_comercialId"))
				on((cli."tableId"=exc."cliente_empresaId" and cli."idto"=346 or
	   				cli."tableId"=exc."cliente_particularId" and cli."idto"=485 or
					cli."tableId"=exc."distribuidorId" and cli."idto"=528) and
	   				fact."agente_comercial"=excag."agente_comercialId" and fact."agente_comercialIdto"=excag."agente_comercialIdto")			left join
	...
	unnest(agente) as agList on(cli.agente_comercial*1000+cli."agente_comercialIdto"=agList or fact.agente_comercial*1000+fact."agente_comercialIdto"=agList)

WHERE
	
	la.cantidad<>0 and
	fact.fecha >= param_fecha_inicio	and
	(param_fecha_fin is null or fact.fecha <= param_fecha_fin+86400)
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION comisiones(integer[], bigint, bigint) OWNER TO dynagent;
...
select
	...
	CASE	when "pedido_de_clienteId" is not null THEN 159
		 	ELSE null END as pedidoidto,
	CASE when "albarán-factura_proveedorId" is not null THEN "albarán-factura_proveedorId"
		 when "albarán_proveedorId" is not null THEN "albarán_proveedorId" 
		 else null END as docid,
	CASE when "albarán-factura_proveedorId" is not null THEN 116
...
CREATE OR REPLACE FUNCTION rotacion_reorden(dias_reposicion double precision,co double precision, ch double precision, slm double precision,delegaciones integer[],segmentoParam character varying,pedido_cliente_especifico integer[])
...
ELSE
	...
	almacen,
	producto,
	CASE WHEN cantidad_venta_periodo is not null and cantidad_venta_periodo>0 THEN sqrt(2*co*ratio_ventas*365/ch) 
	...
	CASE WHEN  cantidad_venta_periodo is null or cantidad_venta_periodo<=0 THEN stock_min
	    ...
	    ELSE loss_inversa(sqrt(2*co*ratio_ventas*365/ch)*(1-slm)/desviacion_std)*desviacion_std+ratio_ventas*lead +stock_min END as reorden,		    
	...
	color	FROM(
		...
		periodo_dias,
		ratio_ventas,
		case  	WHEN dias_reposicion is not null then dias_reposicion 
	    ...
	    from 	estadistica_ventas as ev inner join
	    		...
	    		(select valor_numerico as valor_lr from parametro_numerico where rdn='STOCK_periodo_reposicion') as leadrepo on(valor_lr is not null) inner join				
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION rotacion_reorden(double precision,double precision, double precision, double precision,integer[],character varying,integer[]) OWNER TO dynagent;
...
-- reservar una entrega parcial, es que lineas no entregadas de un pedido servido pero no completo, si que reserven. Lo normal es que no esten servidas por que no hay, no porque se haga en varias veces
  RETURNS SETOF estadistica_rotacion AS
$BODY$
DECLARE
    r estadistica_rotacion%rowtype;    
	...
	ofertaCur 	CURSOR FOR select 	oferta.delegacion,oferta.almacen,oferta.producto,
				...
				FROM
					...
					(	select max(tmp.delegacion) as delegacion,tmp.almacen,max(tmp.producto) as producto,sum(tmp.orden) as orden,max(minimo) as minimo,max(tmp.peso),'NA',tmp.clave_producto,max(tmp.talla) as talla,max(tmp.color) as color
						from rotacion_reorden(dias_reposicion,co,ch,slm,origen,segmento,null) as tmp inner join
...
IF  mensaje_text is not null THEN
	return query select ('el almacen '||mensaje_text || ' no tiene limite stock definido')::varchar;
	return;
END IF;
...
SELECT INTO umbral_redondeo valor_numerico from parametro_numerico where rdn='STOCK_umbral_decimales_redondeo_en_demanda';

IF umbral_redondeo is null then 
	umbral_redondeo:=0.0;
ELSE
	umbral_redondeo:=0.5-umbral_redondeo;
END IF;

IF umbral_redondeo>=1 then 
	umbral_redondeo:=0.49;
END IF;
...
FOR demandaRow IN 	select 	demanda.delegacion,demanda.almacen,demanda.producto,
							case when oferta.orden is null and not ignorar_reservas then demanda.orden+demanda.minimo
								 ...
								 when oferta.orden is not null and oferta.orden<demanda.minimo then demanda.orden+(demanda.minimo-oferta.orden)
					...
					FROM 
						rotacion_reorden(dias_reposicion,co,ch,slm,destinoList,segmento,pedido_cliente_especifico) as demanda  left join
...
c_provisto_demanda:=0;
	...
	LOOP -- loopfeed es para la misma demanda, si la ultima oferta fue insuficiente, vuelvo a buscar una nueva oferta
		...
		IF accion<>'Depreciacion_stock' and (ofertaRow.clave_producto > demandaRow.clave_producto or c_resto_oferta=0) THEN
			...
			RAISE NOTICE 'NOOFER2 dem_pro % ofer_pro % dem_alm % ofer_alm % ofer_q % ',demandaRow.clave_producto,ofertaRow.clave_producto,demandaRow.almacen,ofertaRow.almacen,ofertaRow.orden;		
			-- demanda sin oferta
			IF accion='Provision_demanda_futura' THEN
		...
		IF cantidadRot>0 THEN
			...
			cantidadRot:=round((cantidadRot+umbral_redondeo)::numeric,0);--redondeo para arriba
...
OPEN curLimite FOR select id,maximo,propuesto from limite WHERE propuesto>maximo;
...
--END IF;
return query select ('creada rotacion '||id_rotacion)::varchar;
