...
public class TinyDirectoryModel extends BasicDirectoryModel {
	...
	protected void sort(Vector v) {
		...
		//En la version java 6 el nombre del metodo es sortFiles pero en la java 7 es sort, por lo que hacemos uso de reflection
		Class c = ShellFolder.class;
		Class[] parametertype = new Class[] {List.class};
		Object[] arguments = new Object[] {v};
		try {
			Method concatMethod = c.getMethod("sortFiles", parametertype);
			concatMethod.invoke(c,v);
		}catch (Exception e) {
			System.err.println("Intento con sort");
			try{
				Method concatMethod = c.getMethod("sort", parametertype);
				concatMethod.invoke(c,v);
			}catch(Exception ex){
				ex.printStackTrace();
			}
