...
public abstract class Zip {
	...
	public static boolean zip(File sourceDirectoryFile, File destination) {
		...
		return zipDirectory(sourceDirectoryFile, destination);
	}
	
	private static boolean zipDirectory(File directory, File destinationZip){
		try {
			...
			for (int i=0; i<files.length; i++) {
				File inputFile = new File(directory, files[i]);
