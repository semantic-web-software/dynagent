...
public class EntryTextFileBuilder extends TextFileParser {
	...
	public void buildFile(){
		...
		File source = new File(Constants.JBOSS, "server/default/deploy/jbossweb-tomcat55.sar/ROOT.war/dyna/entry.html");
		if (! source.exists()){
			System.err.println("[Business] Advertencia: No se ha generado el fichero entry.html");
			return;
		}
		//Creamos el fichero destino en una carpeta temporal
		File destination = new File(source.getParentFile(), "/temp/entry.html");
		...
		if (! openFile(source.getAbsolutePath(), destination.getAbsolutePath())){
			System.err.println("[Business] Error: No se ha podido copiar entry.html");
			return;
		}
		...
		while (line != null && ! stop){
			if (line.contains("<param name=\"bns\"")){
				...
				try {
					...
					System.err.println("[Business] Error al intentar copiar una línea al nuevo entry.html");
					close();
					stop = true;
					e.printStackTrace();
				}				
		...
		if (! stop){
			try {
				...
				System.err.println("[] Error: No se ha podido copiar el fichero entry.html generado a la ubicación del original");
				e.printStackTrace();
			}
		}
	...
	private void writeBNSBlocks() {
		...
		while(it.hasNext() && ! stop){
			...
			try {
				...
				System.err.println("[Business] Error al intentar escribir entry.html");
				stop = true;
			}
